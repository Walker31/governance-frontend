...
const ControlAssessment = () => {
  ...
  const handleStatusChange = (controlId, newStatus) => {
    ...
    controlService.updateControl(controlId, { status: newStatus });
    console.log(`Updated control ${controlId} to status ${newStatus}`);
  };
            ...
            <TableBody>
              {controls.map((item) => (
                <TableRow key={item._id} className="hover:bg-muted/50">
                  ...
                  <TableCell className="font-medium">{item.relatedRisks || 'N/A'}</TableCell>
                  <TableCell>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" className="w-full justify-start p-0 h-auto focus-visible:ring-0 focus-visible:ring-offset-0">
                      ...
                      <DropdownMenuContent align="end">
                        {dropdownStatusOptions.map((dropdownStatus) => (
                          <DropdownMenuItem
                            key={dropdownStatus}
                            onSelect={() => handleStatusChange(item._id, dropdownStatus)}
